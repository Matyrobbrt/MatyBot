...
public class TypeBindingPersistentMap extends APersistentMap {
	...
	@Delegate(types = {
			APersistentMap.class, IFn.class, Associative.class, Seqable.class, Counted.class
	}, excludes = Excludes.class)
	...
	public static <T> TypeBindingPersistentMap create(TypeBinding<T> binding, T val) {
		return new TypeBindingPersistentMap(binding, (APersistentMap) binding.toMap(val,
				m -> (Map) PersistentHashMap.create(vectorize(m)), s -> Clojure.read(":" + s)));
	}

	private static final Map<Class<?>, Function<Object, Object[]>> ARRAY_BOXERS = ImmutableMap
			...
			.put(short[].class, o -> ArrayUtils.toObject((short[]) o)).build();
